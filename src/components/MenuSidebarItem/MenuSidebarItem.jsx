...
// import constants
import { MENU_TYPE_BACK, MENU_TYPE_QUIT, STATE_ACTIVE, STATE_DISABLED, STATE_PRESSED } from "@constants";
...
const MenuSidebarItem = memo(function MenuSidebarItem({ type, state = STATE_ACTIVE, onClick }) {
  ...
  useEffect(() => {
    if (type === MENU_TYPE_QUIT || type === MENU_TYPE_BACK) {
  ...
  const pressedClass = isPressed ? styles[`menu-item--${type}-${language}--${STATE_PRESSED}`] : "";
